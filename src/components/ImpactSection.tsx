...
import { Heart, Users, Globe2, ShieldCheck, Github, Video, Code2, ExternalLink, CheckCircle } from 'lucide-react';
        ...
        <div className="bg-[#111a24] p-6 sm:p-8 rounded-3xl border border-[#213548] text-center space-y-6">
          ...
          <div className="pt-4 border-t border-slate-800 flex flex-wrap items-center justify-center gap-6 text-xs text-slate-400">
            ...
            <button 
              type="button"
              onClick={() => {
                setActiveNotice('Pitch Video demo is accessible via the "Explore Interactive Lab" modal in the top header.');
            ...
            <button 
              type="button"
              onClick={() => {
                setActiveNotice('Gemini Multimodal API docs and endpoint schemas are detailed in the Architecture section below.');
                setTimeout(() => setActiveNotice(null), 4500);
              }}
              className="flex items-center gap-1 hover:text-amber-300 transition-colors cursor-pointer"
            >
          ...
          {activeNotice && (
            <div className="mb-4 px-4 py-2.5 rounded-xl bg-amber-500/15 border border-amber-500/30 text-amber-200 text-xs flex items-center justify-center gap-2 animate-fade-in">
