...
import { getLastSignedInAccount, SavedAccount, GOOGLE_CLIENT_ID, signInWithDirectGoogleEmail } from '../firebase';
...
export function GoogleAuthModal({ isOpen, onClose, isDarkMode }: GoogleAuthModalProps) {
  ...
  const handleDirectEmailLogin = (e: React.FormEvent) => {
    ...
    if (!customEmail.trim() || !customEmail.includes('@')) {
      setErrorMessage(isHindiOrRegional ? 'कृपया मान्य गूगल ईमेल दर्ज करें (उदा. tanishque30@gmail.com)' : 'Please enter a valid Google email address (e.g. tanishque30@gmail.com)');
      return;
    }
    setIsSigningIn(true);
    setErrorMessage(null);
    try {
      const signedIn = signInWithDirectGoogleEmail(customEmail.trim());
      setLastAccount(signedIn);
      onClose();
    } catch (err: any) {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
          ...
          <div className="space-y-2">
            ...
            {/* Option: Direct Google Email Sign-In (Ideal for local server & developer testing) */}
            <div className="pt-1">
              <button
                type="button"
                onClick={() => setShowDirectInput(!showDirectInput)}
                className="w-full text-left p-2.5 rounded-xl border border-dashed border-orange-300 dark:border-orange-800/80 hover:border-orange-400 bg-orange-50/40 dark:bg-orange-950/20 hover:bg-orange-50 dark:hover:bg-orange-950/40 transition flex items-center justify-between cursor-pointer focus-visible:ring-2 focus-visible:ring-orange-500 outline-none"
              >
                <div className="flex items-center gap-2.5">
                  <div className="w-7 h-7 rounded-lg bg-orange-100 dark:bg-orange-900/60 text-orange-600 dark:text-orange-400 flex items-center justify-center font-bold text-xs shrink-0">
                    @
                  </div>
                  <div>
                    <p className="text-xs font-bold text-slate-800 dark:text-slate-200 leading-tight">
                      {isHindiOrRegional ? 'जीमेल आईडी सीधे दर्ज करें' : 'Sign in with your Gmail directly'}
                    </p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400">
                      {isHindiOrRegional ? 'लोकल सर्वर और डेवलपमेंट के लिए तुरंत कनेक्ट करें' : 'Fast 1-click access for local server & developers'}
                    </p>
                  </div>
                </div>
                <ChevronRight className={`w-4 h-4 text-slate-400 transition-transform ${showDirectInput ? 'rotate-90' : ''}`} />
              </button>

              {showDirectInput && (
                <form onSubmit={handleDirectEmailLogin} className="mt-2 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-2">
                  <label className="text-[11px] font-semibold text-slate-700 dark:text-slate-300 block">
                    {isHindiOrRegional ? 'अपना गूगल ईमेल दर्ज करें' : 'Enter your Google / Gmail address'}:
                  </label>
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="e.g. tanishque30@gmail.com"
                      className="flex-1 px-3 py-1.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-xs text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-orange-500"
                      autoFocus
                    />
                    <button
                      type="submit"
                      disabled={isSigningIn}
                      className="px-3 py-1.5 rounded-lg bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold shrink-0 transition cursor-pointer"
                    >
                      {isHindiOrRegional ? 'लॉगिन करें' : 'Sign In'}
                    </button>
                  </div>
                </form>
              )}
