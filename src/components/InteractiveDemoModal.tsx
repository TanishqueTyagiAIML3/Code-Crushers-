...
import { getLocalizedDoubtExplanation } from '../utils/localizedFallback';
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          selectedLanguage: selectedChatDialect.name,
          languageCode: selectedChatDialect.code || 'hi-IN',
        })
      });
      const data = await res.json();
      const aiReply = data.reply || getLocalizedDoubtExplanation(userText, selectedChatDialect.id || selectedChatDialect.name);
      setChatMessages(prev => [...prev, { sender: 'ai', text: aiReply }]);
      handleSpeak(aiReply);
    } catch {
      const fallbackReply = getLocalizedDoubtExplanation(userText, selectedChatDialect.id || selectedChatDialect.name);
