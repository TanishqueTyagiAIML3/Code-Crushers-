...
export function MainDashboard({ accessibility, setAccessibility }: MainDashboardProps) {
  ...
  const handleSolveDoubt = async (doubtText: string) => {
    ...
    try {
      ...
      if (!replyText || replyText.trim().length < 10) {
        replyText = getLocalizedDoubtExplanation(doubtText, selectedLanguage.id || selectedLanguage.name);
      }
  ...
  const handleMicButtonClick = () => {
    ...
    if (!SpeechRecognition) {
      setVoiceStatus('Speech recognition not supported in this browser. Please type your doubt.');
